...
- `MemoryEntry`：归档时复制用户当时看到的完整问题，固化永久 `collectionNumber`，并保存查看、擦亮和收藏行为数据。
- `ArchiveState`：`entriesById` 用于按 ID 访问，`entryOrder` 保存新记忆在前的时间线顺序，`nextCollectionNumber` 保存下一条可分配的全局馆藏编号。
...
跨路由动画统一由 `AppRoutes` 外层常驻的 `WarmLightTransitionLayer` 承担。回答页通过 Outlet Context 调用 `startSoftFocusTransition()`，在柔光覆盖峰值执行 `commitDraft()` 并跳转今日入馆页；动画状态仍只属于 React 瞬时 UI，不写入 Zustand 或 Persist。

## 持久化

Zustand Persist 使用 key `zenflow-record-storage-v2`，当前 schema version 为 `3`，`partialize` 仅保留 `draft` 和 `archive`。自定义 `merge` 会补齐旧草稿的浏览历史，把旧 `questionSource: 'ai'` 规范化为 `initialFrameworkSource: 'qwen'`，并在旧草稿已有选中问题时根据其当前候选槽位补出 `selectedCardVariant`。

归档归一化会为缺少馆藏编号的旧条目按旧到新补齐稳定序号，并把下一编号设为现有最大编号加一与已持久计数器两者中的较大值。这样迁移可重复执行，同时删除最高编号后也不会复用已经消耗的序号。开发阶段不读取更早的 `user-record-storage`。
